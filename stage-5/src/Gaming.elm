...
videogame : (Computer -> memory -> List Shape) -> (Computer -> memory -> memory) -> memory -> Program () (Game memory) Msg
videogame viewMemory updateMemory initialMemory =
  let
    init () =
      ( Game E.Visible initialMemory initialComputer
      , Task.perform GotViewport Dom.getViewport
...
gameUpdate updateMemory msg (Game vis memory computer) =
  case msg of
    ...
    GotViewport {viewport} ->
      Game vis memory { computer | screen = toFixedScreen viewport.width viewport.height }

    Resized w h ->
      Game vis memory { computer | screen = toFixedScreen (toFloat w) (toFloat h) }
...
toFixedScreen : Float -> Float -> Screen
toFixedScreen width height =
  { width = 800
  , height = 600
  , top = 300
  , left = -400
